...
function getLatestBlock(req, res, next) {
  blockchain.getLatestBlock()
    .then(latestBlock => res.status(200).json(latestBlock))
    .catch(err => next(err));
}

function findBlockByHash(req, res, next) {
  if (!req.body || !req.body.blockHash) {
    return next(new Error('No block hash provided'));
  }
  blockchain.findBlock(req.body.blockHash)
...
function appendBlock(req, res, next) {
  ...
  blockchain.appendBlock(req.body.companyName)
    .then(newBlock => res.status(200).json(newBlock))
    .catch(err => next(err));
}
